...
(function(window){
    ...
    function filter(input){
        input=input||{};
        var level=String(input.level||"");
        var subjectId=String(input.subjectId||"");
        var term=String(input.term||"");
        return records().filter(function(x){
            return (!level||x.level===level)&&(!subjectId||x.subjectId===subjectId)&&(!term||x.term===term);
        });
    }

    function get(indicatorId){
        var s=source();
        return s&&s.get?s.get(indicatorId):null;
    }
    ...
    function summary(){
        ...
        return {
            total:rows.length,
            valid:v.validRecords||0,
            invalid:v.invalidRecords||0,
            traceable:traceable,
            untraceable:Math.max(rows.length-traceable,0),
            verified:verified,
            levels:LEVELS.slice(),
            terms:TERMS.slice(),
    ...
    function indicatorStatus(indicatorId){
        ...
        return {
            ...
            verification:a&&a.evaluate?a.evaluate(indicatorId):null,
            traceability:t&&t.trace?t.trace(indicatorId):null,
            productionEligible:false,
            prototype:true
        };
    }
    ...
    function validate(){
        var s=summary();
        return {valid:s.invalid===0,total:s.total,invalid:s.invalid,traceable:s.traceable,productionEligible:false,prototype:true};
    }

    window.PacificEducationCurriculumMasterAlignmentWorkspace=Object.freeze({
        name:"PacificEducationCurriculumMasterAlignmentWorkspace",
        version:VERSION,
        levels:LEVELS.slice(),
        terms:TERMS.slice(),
        list:records,
