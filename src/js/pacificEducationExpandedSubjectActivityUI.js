/*
 * Pacific Education
 * Expanded Form 1-7 Pilot Activity UI
 * Version 1.0.0
 *
...
(function(window){
    ...
    function render(){
        ...
        if(!isExpanded(s.level)){
            host.innerHTML="<h2>Form 1–7 Pilot Activities</h2><p>Select Form 1–7 to use the expanded subject activity catalog. Class 1–6 continue using the existing learning flow.</p>";
            return true;
        }

        var records=c.list({level:s.level,subjectId:s.subjectId});
        if(!records.length){
            host.innerHTML="<h2>Form 1–7 Pilot Activities</h2><p>No expanded pilot activity is currently listed for this selection.</p>";
            return true;
        }
